...
const StudentDetailsModal = ({ student, onClose }) => {
    ...
    const studentFields = [
        ...
        { label: "10th Percentage", value: student.per_10 ? `${student.per_10}%` : "N/A" },
        { label: "12th Percentage", value: student.per_12 ? `${student.per_12}%` : "N/A" },
...
// (Omitted for brevity - No changes applied)
// ====================================================================
const AdminEditStudentModal = ({ student, onClose, onSuccess, setToastMessage }) => {
    ...
    const hasDataChanged = (currentData) => {
        ...
        const originalStateWithIds = {
            ...initialFormData,
            session_id: sessions.find(s => s.session_name === initialFormData.session_name)?.session_id ? String(sessions.find(s => s.session_name === initialFormData.session_name)?.session_id) : "",
            program_id: programs.find(p => p.program_name === initialFormData.program_name)?.program_id ? String(programs.find(p => p.program_name === initialFormData.program_name)?.program_id) : "",
        };

        // Keys to compare (all editable fields that go into the API payload)
        const keysToCompare = [
            'rollno', 'name', 'mobile', 'email', 'dob', 'gender',
            'caste', 'address', 'session_id', 'program_id'
        ];

        // Check percentages first as they are numbers/floats
        const per_10_changed = parseFloat(currentData.per_10 || 0) !== parseFloat(originalStateWithIds.per_10 || 0);
        const per_12_changed = parseFloat(currentData.per_12 || 0) !== parseFloat(originalStateWithIds.per_12 || 0);
    ...
    useEffect(() => {
        Promise.all([
            ...
            console.error(err);
            setToastMessage({ type: "error", content: "Failed to load academic options." });
            setLoadingOptions(false);
        });
    }, [setToastMessage]);
    ...
    }, [loadingOptions, sessions, programs, student.session_name, student.program_name, student]);
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        if (!formData.name.trim() || !formData.rollno) {
            setToastMessage({ type: "error", content: "Name and Roll No are required." });
            return;
        }

        // CORE FIX: Perform dirty check. If no changes, stop here.
        if (!hasDataChanged(formData)) {
            setToastMessage({ type: "info", content: "No changes detected. Update cancelled." });
        ...
        try {
            ...
            const updatedStudentNames = {
                session_name: sessions.find(s => String(s.session_id) === formData.session_id)?.session_name || student.session_name,
                program_name: programs.find(p => String(p.program_id) === formData.program_id)?.program_name || student.program_name,
            };
    ...
    if (loadingOptions) {
        return (
            <div className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm z-[1000] p-4">
                <div className="bg-white p-6 rounded-xl shadow-2xl">Loading Edit Form...</div>
            </div>
        );
    }
                ...
                <form onSubmit={handleUpdateSubmit}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        <div className="col-span-1">
                            ...
                            <select name="session_id" value={formData.session_id} onChange={handleInputChange} className="w-full p-2 border rounded-lg bg-white">
                                <option value="">Select Session</option>
                                {sessions.map((s) => (
                                    <option key={s.session_id} value={String(s.session_id)}>{s.session_name}</option>
                                ))}
                            </select>
                        </div>
                        <div className="col-span-1">
                            <label className="block text-sm font-medium">Program</label>
                            <select name="program_id" value={formData.program_id} onChange={handleInputChange} className="w-full p-2 border rounded-lg bg-white">
                        ...
                        <div className="col-span-1">
                            ...
                            <input type="number" step="0.01" name="per_10" value={formData.per_10} onChange={handleInputChange} className="w-full p-2 border rounded-lg" />
                        </div>
                        <div className="col-span-1">
                            <label className="block text-sm font-medium">12th %</label>
                            <input type="number" step="0.01" name="per_12" value={formData.per_12} onChange={handleInputChange} className="w-full p-2 border rounded-lg" />
                        </div>
                        ...
                        <div className="col-span-1">
                            ...
                            <input type="text" name="caste" value={formData.caste} onChange={handleInputChange} className="w-full p-2 border rounded-lg" />
                        </div>
                        <div className="col-span-1">
...
const StudentTable = ({ setToastMessage }) => {
    ...
    const fetchStudents = async (forceFetch = false) => {
        if (!selectedYearId) {
            setToastMessage({ content: "Please select an Academic Year first.", type: "error" });
            setShowStudentList(false);
            return;
        }
        ...
        try {
            ...
            console.error(err);
            setToastMessage({ content: err.response?.data?.message || "Failed to load student data. Is backend running?", type: "error" });
            setIsLoading(false);
            setShowStudentList(false);
        }
    };
    ...
    const handleEditSuccess = (updatedData) => {
        ...
        fetchStudents(true);
        setToastMessage({ type: "success", content: `Student details for ${updatedData.name} updated successfully.` });
    };
    ...
    const deleteStudent = async (userid, studentName) => {
        try {
            ...
            setToastMessage({ type: "success", content: `${studentName}'s record has been deleted.` });
        } catch (err) {
            const errorMessage = err.response?.data?.message || "Failed to delete student record.";
            setToastMessage({ type: "error", content: errorMessage });
        }
    };
    ...
    const exportToExcel = () => {
        ...
        const yearName = academicYears.find(y => y.year_id === selectedYearId)?.year_name || 'Selected';
        const progName = selectedProgramId !== 'all' ? programs.find(p => p.program_id === selectedProgramId)?.program_name : 'All';
        const fileName = `Students_${yearName}_${progName}_${new Date().toISOString().split('T')[0]}.csv`;
        ...
        setToastMessage({ type: "success", content: `Exported ${filteredStudents.length} records to ${fileName}.` });
    };
    ...
    return (
        ...
        <div className="bg-blue-200 py-2 px-4 rounded-xl shadow-md relative pb-2">
            ...
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-3 mb-3">
                ...
                <div className="flex items-end gap-2 flex-wrap">
                    ...
                    <div className="flex flex-col w-36 flex-shrink-0">
                        <label htmlFor="year-select" className="block text-xs font-medium text-gray-700 whitespace-nowrap">Academic Year (Required)</label>
                        <select
                            id="year-select"
                            name="year"
                    ...
                    <div className="flex flex-col w-36 flex-shrink-0">
                        <label htmlFor="program-select" className="block text-xs font-medium text-gray-700 whitespace-nowrap">Program (Optional)</label>
                        <select
                            id="program-select"
                            name="program"
                    ...
                    >
                        {isLoading ? 'Loading...' : showStudentList ? 'Hide Records' : 'Show Records'}
                    </button>
                </div>
            ...
            {showStudentList && (
                <div className="mt-3">
                    {isLoading ? (
                        <p className="text-center text-gray-700 py-4">Loading student data...</p>
                    ) : (
                                        ...
                                        <div className="max-h-64 overflow-y-auto hide-scrollbar">
                                            {filteredStudents.map((student, index) => (
                                                ...
                                                >
                                                    ...
                                                    <div>{index + 1}.</div>
                                                    {/* Name */}
                                                    <div className="font-semibold break-words">{student.name}</div>
                                                    {/* Roll No. */}
                                                    <div>{student.rollno || "N/A"}</div>
                                                    {/* Program Name */}
                                                    <div className="break-words">{student.program_name || "N/A"}</div>
                                                    {/* Mobile No. */}
                                                    ...
                                                    {/* Modified By */}
                                                    <div className="break-words pr-2">{student.modified_by || "N/A"}</div>

                                                    {/* Last Modified */}
                                                    <div className="whitespace-nowrap">
                                                        {student.mod_time
                                                            ? new Date(student.mod_time).toLocaleString().replace(' ', '\u00A0')
                                                            : "N/A"}
                                                    </div>
            ...
            {showConfirmModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm z-[1000]">
                    <div className="bg-white w-full max-w-md rounded-xl shadow-2xl p-6 animate-fadeIn">
                        ...
                        <p className="text-gray-600 mb-6">Do you really want to delete this student record? This cannot be undone.</p>
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setShowConfirmModal(false)} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 transition">Cancel</button>
                            <button onClick={confirmAction} className="px-4 py-2 rounded-lg bg-red-500 text-white hover:bg-red-600 transition">Confirm Delete</button>
                        </div>
                    </div>
                </div>
            )}
